...
export default async function handler(req, res) {
  ...
  // Same free deliverability guard used by signup (see
  // _emailValidation.js and auth-lockout.js's 'validate-email' action):
  // syntax + DNS MX/A/AAAA + disposable-domain blocklist. Checked before
  // either path below so a fake or can't-receive-mail address never
  // reaches the profiles lookup, never gets added to org_members, and
  // never triggers a real Supabase invite email.
  const emailCheck = await validateEmailDeliverable(normalizedEmail)
  if (!emailCheck.valid) {
