...
async function processOrg(supabase, org, today, resendApiKey, digestFromAddress, summary) {
  ...
  for (const [userId, prefs] of prefsByUser) {
    ...
    if (resendApiKey) {
      const result = await sendDigestEmail(resendApiKey, digestFromAddress, email, org.name, sections)
      if (result.ok) {
        summary.emailsSent += 1
      } else {
        logServerError(`daily-digest:send:${org.id}:${userId}`, new Error(result.error))
        summary.errors.push(`${org.name}: digest email to ${email} failed — ${result.error}`)
      }
    }
  }
}
...
// member of one org shouldn't take down the whole run for every other
// org/member in the same cron invocation. The caller (processOrg above)
// is what actually decides whether that counts as a real failure worth
// surfacing in summary.errors and error_log, rather than this function
// silently succeeding on a rejected request the way it used to (Resend
// returning a non-2xx, e.g. an invalid API key, used to still increment
// emailsSent -- caught via a live 403 during setup).
async function sendDigestEmail(apiKey, fromAddress, toEmail, orgName, sections) {
  const html = `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
      <h2 style="margin-bottom: 4px;">${escapeHtml(orgName)} — daily digest</h2>
