...
export default function Settings() {
  ...
  const [wiseLinkSaving, setWiseLinkSaving] = useState(false)
  const [wiseLinkError, setWiseLinkError] = useState('')
  const [wiseLinkSaved, setWiseLinkSaved] = useState(false)

  const [invoicePrefixSaving, setInvoicePrefixSaving] = useState(false)
  ...
  const loadStripeStatus = useCallback(async () => {
    if (!activeOrgId || !isAdmin) return
    try {
      setStripeStatus(await getStripeStatus(activeOrgId))
    } catch (err) {
  ...
  const handleStripeConnect = async (e) => {
    e.preventDefault()
    if (!stripeSecretKeyInput.trim() || !stripeWebhookSecretInput.trim()) return
    setStripeBusy(true)
    setStripeError('')
    setStripeNotice('')
    try {
      await connectStripe(activeOrgId, stripeSecretKeyInput.trim(), stripeWebhookSecretInput.trim())
      setStripeSecretKeyInput('')
      setStripeWebhookSecretInput('')
      setStripeNotice('Connected — secret key verified with Stripe.')
  ...
  const handleStripeDisconnect = async () => {
    ...
    setStripeNotice('')
    try {
      await disconnectStripe(activeOrgId)
      await loadStripeStatus()
    } catch (err) {
      setStripeError(err.message)
    }
  ...
  const handleSaveWiseLink = async (e) => {
    e.preventDefault()
    setWiseLinkError('')
    setWiseLinkSaved(false)
    setWiseLinkSaving(true)
    const { error: updateError } = await supabase
      .from('organizations')
      .update({ wise_payment_link: wiseLink.trim() || null })
      .eq('id', activeOrgId)
    setWiseLinkSaving(false)
    if (updateError) {
      setWiseLinkError(friendlyError(updateError))
      return
    }
    setWiseLinkSaved(true)
  }

  const handleSaveInvoicePrefix = async (e) => {
    e.preventDefault()
    setInvoicePrefixError('')
    setInvoicePrefixSaved(false)
    setInvoicePrefixSaving(true)
    const { error: updateError } = await supabase
      .from('organizations')
      .update({ invoice_prefix: invoicePrefix.trim() || 'INV' })
      .eq('id', activeOrgId)
    setInvoicePrefixSaving(false)
    if (updateError) {
      setInvoicePrefixError(friendlyError(updateError))
      return
    }
  ...
  const handleSaveBilling = async (e) => {
    ...
    const { error: updateError } = await supabase
      .from('organizations')
      .update({
        biller_name: billerName.trim() || null,
        biller_company: billerCompany.trim() || null,
        biller_address: billerAddress.trim() || null,
        default_hourly_rate: defaultHourlyRate.trim() && Number.isFinite(parsedRate) && parsedRate > 0 ? parsedRate : null,
      })
      .eq('id', activeOrgId)
    setBillingSaving(false)
    if (updateError) {
      setBillingError(friendlyError(updateError))
      return
    }
  ...
  const handleSavePrefs = async (e) => {
    ...
    if (upsertError) {
      setPrefsError(friendlyError(upsertError))
      return
    }
    setPrefsSaved(true)
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {loadError && (
        <p className="text-sm rounded-md px-3 py-2 mb-6" style={{ background: 'var(--tally-alert-soft)', color: 'var(--tally-alert-text)' }} role="alert">
          {loadError}
        </p>
      )}

      {!isAdmin && (
        <p className="text-sm rounded-md px-3 py-2 mb-6" style={{ background: 'var(--tally-progress-soft)', color: 'var(--ink)' }}>
          Only workspace owners/admins can change these settings. You can view them here.
        </p>
      )}

      <div className="grid gap-6 sm:grid-cols-2">
      <form onSubmit={handleSaveProfile} className="rounded-lg border p-5 space-y-4" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
        <div>
      ...
      <form onSubmit={handleSaveInvoicePrefix} className="rounded-lg border p-5 space-y-4" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
        <div>
          <h2 className="font-display font-bold text-lg mb-1">Invoice numbering</h2>
          <p className="text-xs" style={{ color: 'var(--ink-muted)' }}>
            New invoices are numbered automatically — change the prefix used for all of them here.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-xs mb-2" style={{ color: 'var(--ink-muted)' }}>
            e.g. {invoicePrefix || 'INV'}-0001, {invoicePrefix || 'INV'}-0002…
          </p>
          <input
            id="invoice-prefix"
            type="text"
        ...
        {isAdmin && (
          <button
            type="submit"
            disabled={invoicePrefixSaving}
            className="rounded-md px-4 py-2 text-sm font-medium disabled:opacity-60"
            style={{ background: 'var(--ink)', color: 'var(--panel)' }}
          >
            {invoicePrefixSaving ? 'Saving…' : 'Save'}
          </button>
        )}
      </form>
      ...
      <div className="rounded-lg border p-5 space-y-4" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
        <div>
      ...
      <div className="mt-8">
        <h2 className="font-display font-bold text-lg mb-1">Payments</h2>
        <p className="text-xs mb-4" style={{ color: 'var(--ink-muted)' }}>
          Your invoice identity, and how clients pay you — with auto-reconciliation where it's available.
        </p>
        <div className="grid gap-6 sm:grid-cols-2">
      <form onSubmit={handleSaveBilling} className="rounded-lg border p-5 space-y-4" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
        <div>
          <h2 className="font-display font-bold text-lg mb-1">Billing</h2>
          <p className="text-xs" style={{ color: 'var(--ink-muted)' }}>
            Your own business identity, shown as the letterhead on every invoice PDF (top right, next to the
            invoice number). Leave any of these blank and that line just doesn't show up on the invoice.
          </p>
        </div>

        <div>
          <label htmlFor="biller-name" className="block text-sm font-medium mb-1">Name</label>
          <input
            id="biller-name"
            type="text"
            value={billerName}
            onChange={(e) => setBillerName(e.target.value)}
            placeholder="Your name or your business's name"
            disabled={!isAdmin}
        ...
        <div>
          <label htmlFor="biller-company" className="block text-sm font-medium mb-1">Company (optional)</label>
          <input
            id="biller-company"
            type="text"
            value={billerCompany}
            onChange={(e) => setBillerCompany(e.target.value)}
            placeholder="e.g. a company name separate from the name above"
            disabled={!isAdmin}
            className="w-full rounded-md border px-3 py-2 text-sm disabled:opacity-60"
            style={{ borderColor: 'var(--border)' }}
          />
        ...
        <div>
          <label htmlFor="biller-address" className="block text-sm font-medium mb-1">Address (optional)</label>
          <textarea
            id="biller-address"
            value={billerAddress}
        ...
        <div>
          <label htmlFor="default-hourly-rate" className="block text-sm font-medium mb-1">Default hourly rate (optional)</label>
          <input
          ...
          <p className="text-xs mt-1" style={{ color: 'var(--ink-muted)' }}>
            Used to price logged time on invoices. Any project can override this with its own rate; leave blank
            and time tracking still works, just without a dollar amount attached.
          </p>
        </div>
      ...
      {isAdmin && (
        <div className="rounded-lg border p-5 space-y-4" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
          <div>
            <h2 className="font-display font-bold text-lg mb-1">Stripe payments</h2>
            <p className="text-xs" style={{ color: 'var(--ink-muted)' }}>
              Workspace-wide, admin-only, same as Wise auto-reconciliation above — an alternative for clients who'd
              rather pay by card than bank transfer. Once connected, every invoice gets its own "Generate Stripe
              payment link" button (own page, next to the Wise link) — Stripe pushes a real-time notification the
              moment that link gets paid, so the invoice gets marked paid immediately and exactly, no reference text
              to match or daily wait involved.
            </p>
          </div>

          {stripeStatus?.connected ? (
            <div className="space-y-3">
              <p className="text-sm" style={{ color: 'var(--tally-done)' }}>Connected — secret key verified.</p>
              {stripeStatus.lastError && (
                <p className="text-xs" style={{ color: 'var(--ink-muted)' }}>Last note: {stripeStatus.lastError}</p>
              )}
              <button
                type="button"
            ...
            <form onSubmit={handleStripeConnect} className="space-y-3">
              <div>
                <p className="text-xs mb-2" style={{ color: 'var(--ink-muted)' }}>
                  Two things needed, both from your Stripe Dashboard: your <span className="font-medium">secret
                  key</span> (Developers → API keys), and a <span className="font-medium">webhook signing
                  secret</span> — register this exact URL under Developers → Webhooks first, listening for
                  {' '}<span className="font-mono">checkout.session.completed</span>, and Stripe will give you the
                  signing secret to paste below:
                </p>
                <p className="text-xs font-mono rounded-md border px-3 py-2 mb-3 break-all" style={{ borderColor: 'var(--border)', background: 'var(--panel-sunken)' }}>
                  {stripeWebhookUrl(activeOrgId)}
                </p>
                <label htmlFor="stripe-secret-key" className="sr-only">Stripe secret key</label>
                <input
                  id="stripe-secret-key"
                  type="password"
                  value={stripeSecretKeyInput}
                  onChange={(e) => setStripeSecretKeyInput(e.target.value)}
                  placeholder="Paste your Stripe secret key (sk_live_… or sk_test_…)"
                  className="w-full rounded-md border px-3 py-2 text-sm mb-2"
                  style={{ borderColor: 'var(--border)' }}
                />
                <label htmlFor="stripe-webhook-secret" className="sr-only">Stripe webhook signing secret</label>
                <input
                  id="stripe-webhook-secret"
                  type="password"
                  value={stripeWebhookSecretInput}
                  onChange={(e) => setStripeWebhookSecretInput(e.target.value)}
                  placeholder="Paste the webhook signing secret (whsec_…)"
                  className="w-full rounded-md border px-3 py-2 text-sm"
                  style={{ borderColor: 'var(--border)' }}
                />
              </div>
              <button
                type="submit"
                disabled={stripeBusy || !stripeSecretKeyInput.trim() || !stripeWebhookSecretInput.trim()}
                className="rounded-md px-4 py-2 text-sm font-medium disabled:opacity-60"
                style={{ background: 'var(--ink)', color: 'var(--panel)' }}
              >
                {stripeBusy ? 'Connecting…' : 'Connect'}
              </button>
            </form>
          )}
      ...
      <form onSubmit={handleSaveWiseLink} className="rounded-lg border p-5 space-y-4" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
        <div>
          <h2 className="font-display font-bold text-lg mb-1">Wise payment</h2>
          <p className="text-xs" style={{ color: 'var(--ink-muted)' }}>
            Your permanent Wise Business payment link (from Wise → Payments → "Your open link"). It gets
            embedded on every invoice, along with a reminder for the client to reference the invoice number
            when they pay.
          </p>
        </div>
        ...
        {isAdmin && (
          <button
            type="submit"
            disabled={wiseLinkSaving}
            className="rounded-md px-4 py-2 text-sm font-medium disabled:opacity-60"
            style={{ background: 'var(--ink)', color: 'var(--panel)' }}
          >
            {wiseLinkSaving ? 'Saving…' : 'Save'}
          </button>
        )}
      </form>
          ...
          {wiseStatus?.connected ? (
            <div className="space-y-3">
              {wiseStatus.supported === false ? (
                <p className="text-sm rounded-md px-3 py-2" style={{ background: 'var(--tally-alert-soft)', color: 'var(--tally-alert-text)' }}>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  style={{ borderColor: 'var(--tally-alert)', color: 'var(--tally-alert)' }}
                >
                  {wiseBusy ? 'Disconnecting…' : 'Disconnect'}
                </button>
              </div>
            </div>
          ) : (
      ...
      <div className="rounded-lg border p-5 space-y-4 mt-8" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
        <div>
