  ...
  const handleSubmit = async (e) => {
    ...
    const { data: inserted, error: insertError } = await supabase
      ...
      .insert({
        ...
        hourly_rate: hourlyRate.trim() && Number.isFinite(parsedRate) && parsedRate > 0 ? parsedRate : null,
    ...
    // Same best-effort philosophy as members and attachments above -- the
    // project already exists at this point, so a template failure
    // shouldn't block navigation. Tasks can always be added by hand (or
    // the template re-applied) from the project's own page afterward.
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6" noValidate>
        <div className="rounded-lg border p-5" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
          ...
          <div className="mb-4">
            <label htmlFor="proj-hourly-rate" className="block text-sm font-medium mb-1">Hourly rate override (optional)</label>
            <input
              id="proj-hourly-rate"
              type="number"
              min="0"
              ...
              placeholder="Leave blank to use the org's default rate"
        ...
        {isAdmin && (
          <div className="rounded-lg border p-5" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
            <h2 className="font-display font-bold text-lg mb-1">Start from a template</h2>
            <p className="text-sm mb-3" style={{ color: 'var(--ink-muted)' }}>
              Optional — pre-fills the task list. Can also be applied later from the project's own
              page.
            </p>
            <TemplatePicker
              orgId={activeOrgId}
