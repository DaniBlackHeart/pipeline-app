...
export function AuthProvider({ children }) {
  ...
  const [profile, setProfile] = useState(null) // { id, full_name, nickname, email, onboarding_completed_at } | null
  ...
  const loadProfile = useCallback(async (userId) => {
    ...
    const { data, error } = await supabase
      .from('profiles')
      .select('id, full_name, nickname, email, onboarding_completed_at')
      .eq('id', userId)
      .maybeSingle()
  ...
  const value = {
    ...
    // True once we know who this person is and their profile row hasn't
    // been marked onboarded yet. Checked after needsMfaChallenge so the
    // 2FA step still comes first when both apply. `profile` starts out
    // null while it's loading, which deliberately reads as "don't know
    // yet" (false) rather than flashing the onboarding redirect for
    // everyone on every load.
    needsOnboarding: Boolean(profile && !profile.onboarding_completed_at),
  }
